...
class LifecyclePartBuilder implements TaskPartBuilderInterface
{
    /**
     * @var \SprykerSdk\Sdk\Infrastructure\Builder\TaskYamlBuilder\TaskPartBuilder\TaskPartBuilderInterface
     */
    protected TaskPartBuilderInterface $placeholderPartBuilder;

    /**
     * @param \SprykerSdk\Sdk\Infrastructure\Builder\TaskYamlBuilder\TaskPartBuilder\TaskPartBuilderInterface $placeholderPartBuilder
     */
    ...
    protected function createInitializedEventData(
        ...
        if (!isset($taskData['lifecycle'][LifecycleEnum::TYPE_INITIALIZED])) {
            return new InitializedEventData();
        }

        $eventData = $taskData['lifecycle'][LifecycleEnum::TYPE_INITIALIZED];
    ...
    protected function createUpdatedEventData(
        ...
        $taskData = $criteriaDto->getTaskData();
        if (!isset($taskData['lifecycle'][LifecycleEnum::TYPE_UPDATED])) {
            return new UpdatedEventData();
        }

        $eventData = $taskData['lifecycle'][LifecycleEnum::TYPE_UPDATED];
        $commands = $this->buildCommands($eventData);
        $placeholders = $this->buildPlaceholders($eventData, $criteriaDto);
        $files = $this->buildFiles($eventData);
    ...
    protected function createRemovedEventData(
        ...
        $taskData = $criteriaDto->getTaskData();
        if (!isset($taskData['lifecycle'][LifecycleEnum::TYPE_REMOVED])) {
            return new RemovedEventData();
        }

        $eventData = $taskData['lifecycle'][LifecycleEnum::TYPE_REMOVED];
        $commands = $this->buildCommands($eventData);
        $placeholders = $this->buildPlaceholders($eventData, $criteriaDto);
        $files = $this->buildFiles($eventData);
